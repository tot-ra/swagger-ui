...
//                log("li_content_dom_id " + li_content_dom_id);
	...
	toggleEndpointListForResource: function(resource) {
		var elem = $('li#resource_' + Docs.escapeResourceName(resource) + ' .toggleable');
		if (elem.is(':visible')) {
  ...
  MainView = (function(_super) {
    ...
    MainView.prototype.addResource = function(resource) {
      ...
      resourceView = new ResourceView({
		...
		var resourceHTML = resourceView.render().el;

		$('.heading:first', resourceHTML).after('<p>'+resource.description + '</p><h3 class="toggleable" style="display: none;">Operation models</h3>' );

		$.each(resource.models, function(i,v){
			modelHTML = '<table class="toggleable model" style="display: none;"><caption>'+ v.id + '</caption>';
			$.each(v.properties, function(i2,v2){
//				console.log(v2);
				modelHTML +='<tr><td>'+ i2 +'</td><td>'+ v2.type +'</td><td>'+ v2.description+'</td></tr>';
			});
			modelHTML +='</table>';
			//<pre>'+JSON.stringify(v.properties, null,2)+'</pre>';
					$('.endpoints', resourceHTML).before(
							modelHTML
					);
				});
		$('.endpoints', resourceHTML).before(
				'<h3 style="display: none;" class="toggleable">Operations</h3>'
		);

      return $('#resources').append(resourceHTML);
    };
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.render = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
		...
		var errorsHTML = '<h4>Error messages</h4><table>';
		$.each(this.model.errorResponses, function(i,v){
			errorsHTML +='<tr><td>'+ v.code +'</td><td>'+ v.reason+'</td></tr>';
		});
		errorsHTML+='</table>';

		$('.content', this.el).append(errorsHTML);
      return this;
    };
